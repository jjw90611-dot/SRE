...
# ==========================================
# 1. 페이지 설정 및 UI 구성 (가장 먼저 호출되어야 함)
# ==========================================
st.set_page_config(
...
# 2순위: Streamlit Secrets에서 확인 (에러 방지를 위해 try-except 적용)
if not api_key:
    try:
...
# 3순위: 사이드바에서 직접 입력받기
if not api_key:
    api_key = st.sidebar.text_input("🔑 Groq API Key를 입력해주세요", type="password")

# API 키가 없으면 실행 중단 및 안내 메시지 출력
if not api_key:
    st.info("👈 좌측 사이드바에 Groq API Key를 입력해야 서비스가 시작됩니다.")
    st.stop()
...
# ==========================================
# 3. 시스템 프롬프트 정의 (r-string 적용으로 수식 깨짐 방지)
# ==========================================
# r""" """ (Raw String)을 사용하여 LaTeX 수식의 백슬래시(\)가 깨지지 않도록 보호합니다.
SYSTEM_PROMPT = r"""당신은 첨단 소재 회사 포스코 퓨처엠 임직원을 위한 '포항시 부동산 맞춤형 AI 컨설턴트'입니다.
...
# ==========================================
# 5. 사용자 입력 및 AI 응답 처리 (스트리밍 안정성 강화)
# ==========================================
if prompt := st.chat_input("포항 부동산에 대해 궁금한 점을 물어보세요! (예: 대잠동 신축 아파트 추천해줘)"):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            # 에러 발생 시 대화 기록에서 마지막 사용자 질문 제거 (재시도 가능하도록)
