...
# 키가 없으면 입력창 띄우기 (성공 메시지는 제거하여 화면을 깔끔하게 유지)
if not groq_api_key:
...
def get_real_estate_data(lawd_cd, apt_name):
    ...
    # 최신 국토교통부 아파트매매 실거래 상세 자료 API URL로 변경
    ...
    for ymd in months_to_check:
        ...
        try:
            ...
            # 최신 API 규격에 맞게 영문 태그명으로 수정 (aptNm, dealAmount, excluUseAr, dealDay, floor)
            for item in root.findall('.//item'):
                api_apt_name = item.find('aptNm').text if item.find('aptNm') is not None else ""
                
                # API에서 온 아파트 이름도 공백 제거 후 비교
                if search_name in api_apt_name.replace(" ", ""):
                    price = item.find('dealAmount').text.strip()
                    area = float(item.find('excluUseAr').text.strip())
                    pyeong = round(area / 3.3058, 1) # 평수 변환
                    day = item.find('dealDay').text.strip()
